	...
	export default {
		...
		methods: {
			attachUser(user) {
				...
				function(){
					this.attaching = true
					this.$http.post('permissions/'+this.permission.id+'/users/'+this.user.user_id+'/attach')
					.then(response => {
					...
					.catch(err => {
						this.attaching = false
						swal('Error '+err.status+': '+err.statusText,'Failed to Attach Permission to user.','error')
					})
				}.bind(this))
			}
		},
